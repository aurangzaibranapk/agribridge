...
export function GalleryGrid({ items }: { items: GalleryItem[] }) {
  ...
  if (items.length === 0) {
    return <p className="text-center text-sm text-surface-400 dark:text-surface-500">No gallery items yet.</p>;
  }
    ...
    <div>
      ...
      {lightboxItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setLightboxItem(null)}>
          <button className="absolute right-4 top-4 text-white" onClick={() => setLightboxItem(null)} aria-label="Close">
            <X className="h-6 w-6" />
          </button>
