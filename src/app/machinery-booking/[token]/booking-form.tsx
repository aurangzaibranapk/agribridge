...
import { submitPublicMachineryRequest, type PublicBookingState } from "@/actions/public-machinery-booking";
...
export function PublicBookingForm({ token, farmerName }: { token: string; farmerName: string }) {
  const [state, formAction] = useFormState(submitPublicMachineryRequest, initialState);
  ...
  return (
    <div dir="rtl" className="rounded-card border border-surface-200 bg-white p-4 shadow-card">
      <h2 className="flex items-center justify-end gap-2 font-display text-sm font-semibold text-surface-900">
        السلام علیکم، {farmerName} <Tractor className="h-4 w-4 text-brand-600" />
      </h2>
      <p className="mt-1 text-right text-xs text-surface-500">اپنا اکاؤنٹ محفوظ رکھنے کے لیے نیچے سے کوئی طریقہ منتخب کریں، یا سیدھا فارم بھر دیں۔</p>
      ...
      <div className="my-4 flex items-center gap-3">
        ...
        <span className="text-xs text-surface-400">یا سیدھا فارم بھریں</span>
        <div className="h-px flex-1 bg-surface-200" />
      </div>

      {state.error && <p className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-right text-xs text-red-700">{state.error}</p>}
      <form action={formAction} className="mt-3 space-y-3">
        ...
        <div>
          ...
          <input type="text" defaultValue={farmerName} disabled className="mt-1 w-full rounded-lg border border-surface-200 bg-surface-50 p-2 text-sm text-surface-500" />
        </div>
