...
export function StaticPageForm({ page }: { page: any }) {
  ...
  return (
    <form action={formAction} className="max-w-2xl space-y-4 rounded-card border border-surface-200 bg-white p-6 shadow-card dark:border-surface-800 dark:bg-surface-900">
      ...
      {state.success && <p className="rounded-lg bg-brand-50 px-3 py-2 text-sm text-brand-700 dark:bg-brand-900/30 dark:text-brand-300">Saved.</p>}
      <div><Label htmlFor="title">Title</Label><Input id="title" name="title" defaultValue={page.title} /></div>
      <div><Label htmlFor="content">Content</Label><Textarea id="content" name="content" rows={16} defaultValue={page.content} /></div>
      <SubmitButton />
    </form>
  );
}
