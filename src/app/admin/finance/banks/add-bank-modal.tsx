    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl dark:bg-surface-900">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900 dark:text-white">Add New Bank</h3>
          <button onClick={onClose} className="text-surface-400 hover:text-surface-700"><X className="h-5 w-5" /></button>
        </div>
        {state.error && <p className="mb-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{state.error}</p>}
        {state.success && <p className="mb-2 rounded-lg bg-brand-50 px-3 py-2 text-xs text-brand-700">Bank add ho gaya.</p>}
        <form action={formAction} encType="multipart/form-data" className="space-y-3">
          <div>
            <label className="text-xs font-medium text-surface-600">Bank Logo</label>
          ...
          <div>
            ...
            <input name="name" required placeholder="e.g. HBL, UBL, Bank Alfalah" className="mt-1 w-full rounded-lg border border-surface-200 p-2 text-sm" />
          </div>
          <div>
            <label className="text-xs font-medium text-surface-600">Account Number (optional)</label>
          ...
          <div>
            <label className="text-xs font-medium text-surface-600">Opening Balance</label>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="rounded-lg border border-surface-200 px-3 py-2 text-sm text-surface-600 hover:bg-surface-50">Cancel</button>
            <SubmitButton />
          </div>
        </form>
      </div>
