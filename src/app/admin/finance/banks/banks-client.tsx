    ...
    <div>
      ...
      {banks.length === 0 ? (
        <div className="rounded-card border border-dashed border-surface-200 bg-white p-10 text-center dark:border-surface-800 dark:bg-surface-900">
          <p className="mb-3 text-surface-400">Koi bank add nahi hui abhi.</p>
          <button onClick={() => setShowAddModal(true)} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">Add First Bank</button>
        </div>
      ) : (
            ...
            <div key={bank.id} className="rounded-card border border-surface-200 bg-white p-4 shadow-card transition hover:shadow-md dark:border-surface-800 dark:bg-surface-900">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex gap-1">
                  <button onClick={() => setEditingBank(bank)} className="rounded-lg p-1.5 text-surface-500 hover:bg-surface-50" title="Edit">
...
function DeleteButton({ bankId }: { bankId: string }) {
  ...
  return (
    ...
    >
      ...
      <button type="submit" className="rounded-lg p-1.5 text-red-500 hover:bg-red-50" title="Delete">
