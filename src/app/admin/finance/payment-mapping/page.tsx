...
import { WaselaToggle } from "./wasela-toggle";
import { getWaselaIntegrationStatus } from "@/actions/wasela-integration";
...
export default async function PaymentMappingPage() {
  ...
  const [{ data: mappings }, { data: accounts }, waselaEnabled] = await Promise.all([
    supabase.from("payment_method_account_map").select("payment_method, finance_account_id").order("payment_method"),
    supabase.from("finance_accounts").select("id, name, account_type").eq("is_active", true).order("account_type"),
    getWaselaIntegrationStatus(),
  ]);
  ...
  return (
    <div>
      ...
      <WaselaToggle currentlyEnabled={waselaEnabled} />
    </div>
  );
}