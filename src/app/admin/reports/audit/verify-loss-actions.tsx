    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900">Loss Reject Karein</h3>
        ...
        <form action={formAction} className="space-y-2">
          ...
          <textarea name="rejection_reason" required rows={3} placeholder="Reject karne ki wajah (jaise: asal mein loss nahi thi)" className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
...
function ReducedRateModal({ lossId, onClose }: { lossId: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900">Kam Rate Pe Nikalna Hai</h3>
          <button onClick={onClose} className="text-surface-400 hover:text-surface-700"><X className="h-5 w-5" /></button>
        </div>
        ...
        <form action={formAction} className="space-y-2">
          ...
          <div>
            <label className="text-xs text-surface-500">Naya (Kam) Rate Per Unit (Rs)</label>
            <input type="number" step="0.01" name="reduced_rate" required placeholder="Naya rate" className="mt-1 w-full rounded-lg border border-surface-200 p-2 text-sm" />
          </div>
          <button type="submit" className="w-full rounded-lg bg-blue-600 py-2 text-sm font-medium text-white hover:bg-blue-700">Confirm Kam Rate</button>
        </form>
      </div>
    </div>
  );
