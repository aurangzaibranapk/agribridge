...
export function BranchStatementClient(props: Props) {
  ...
  function statementText() {
    const lines = [
      ...
      "",
      `Total Order Charges: Rs ${props.totalDebit.toLocaleString()}`,
      `Total Payments/Advance: Rs ${props.totalCredit.toLocaleString()}`,
  ...
  function handleDownload() {
    ...
    a.href = url;
    a.download = `${props.branchName.replace(/\s+/g, "-")}-statement-${props.startDate}-to-${props.endDate}.txt`;
    a.click();
  ...
  function handleEmail() {
    window.location.href = `mailto:?subject=${encodeURIComponent(`${props.branchName} Statement`)}&body=${encodeURIComponent(statementText())}`;
  }
    ...
    <div>
      ...
      <div className="rounded-card border border-surface-200 bg-white p-5 shadow-card dark:border-surface-800 dark:bg-surface-900">
        <div className="mb-4 border-b border-surface-100 pb-3 dark:border-surface-800">
          <h2 className="font-display text-lg font-semibold text-surface-900 dark:text-white">{props.branchName} {props.district ? `(${props.district})` : ""}</h2>
