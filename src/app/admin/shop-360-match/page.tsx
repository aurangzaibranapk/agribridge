...
export default async function Shop360MatchPage({ searchParams }: { searchParams?: { shop_id?: string; period?: string; from?: string; to?: string } }) {
  ...
  // Final server-side guard: non-broad users may never load another shop.
  ...
  return <div className="space-y-5">
    ...
    <Card><h3 className="mb-3 font-semibold">Paisa / Value Kahan Hai?</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 text-sm"><div><p className="text-xs text-surface-500">Stock — Selling Rate</p><b>{money(snap.stock.value)}</b></div><div><p className="text-xs text-surface-500">Cash Sale</p><b>{money(cashMethod)}</b></div><div><p className="text-xs text-surface-500">Bank / Digital Sale</p><b>{money(digital)}</b></div><div><p className="text-xs text-surface-500">Khata Sale in selected day</p><b>{money(khataMethod)}</b></div><div><p className="text-xs text-surface-500">Approved Expenses</p><b>{money(snap.flow.expenses.total)}</b></div><div><p className="text-xs text-surface-500">Deposit Pending Finance</p><b>{money(snap.deposits.pendingDeposits)}</b></div><div><p className="text-xs text-surface-500">Finance Verified Deposit</p><b>{money(snap.deposits.approvedDeposits)}</b></div><div><p className="text-xs text-surface-500">Remaining Outstanding</p><b>{money(snap.deposits.outstanding)}</b></div></div><p className="mt-3 text-[11px] text-surface-400">Ye rows state visibility ke liye hain; incomplete shop-attribution ki wajah se inhein jor kar fake Full Match total nahi banaya gaya.</p></Card>
