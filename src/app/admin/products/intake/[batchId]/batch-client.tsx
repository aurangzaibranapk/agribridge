        ...
        <Card>
          <div className="flex flex-wrap items-center gap-3">
            <Button type="button" onClick={() => setScanOpen(true)}>
              <span className="inline-flex items-center gap-1.5">
                <ScanLine className="h-4 w-4" /> Barcode scan karein
              </span>
            </Button>

            <form action={blankAction}>
              <input type="hidden" name="batch_id" value={batchId} />
              <Submit label="Bina barcode ke" icon={<Plus className="h-4 w-4" />} variant="secondary" />
            </form>

            <span className="text-xs text-surface-500">
              {items.length} qatarein · <strong className="text-emerald-700">{ready} tayyar</strong>
              {draft > 0 && ` · ${draft} adhoori`}
            </span>
          </div>
          <Msg state={scanState} />
      ...
      {items.length === 0 ? (
        <Card>
          <p className="text-sm text-surface-500">
            Abhi kuch nahi. Dabba haath mein lein aur <strong>Barcode scan karein</strong> dabayein.
          </p>
        </Card>
      ) : (
        items.map((item, idx) => (
          <ItemCard key={item.id} n={idx + 1} item={item} categories={categories} done={done} />
        ))
      )}
      ...
      {!done && items.length > 0 && (
        <Card>
          <form action={approveAction}>
            <input type="hidden" name="batch_id" value={batchId} />
            <div className="flex flex-wrap items-center gap-3">
              <Submit label={`${ready} products manzoor karein`} icon={<CheckCircle2 className="h-4 w-4" />} />
              {draft > 0 && (
                <span className="inline-flex items-center gap-1 text-xs text-amber-800">
                  <AlertTriangle className="h-3.5 w-3.5" />
                  {draft} qatarein adhoori hain (naam ya sale rate baqi) — wo charhengi nahi
                </span>
              )}
            </div>
      ...
      {done && (
        <Card>
          <p className="flex items-center gap-2 text-sm text-emerald-800">
            <CheckCircle2 className="h-4 w-4" /> Ye chakkar manzoor ho chuka hai — maal warehouse mein aa gaya.
          </p>
        </Card>
      )}
    </div>
...
function ItemCard({
  ...
  async function pickPhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setUploadError(err instanceof Error ? err.message : "Tasveer charh nahi saki.");
    } finally {
      setUploading(false);
    }
  }
    ...
    <Card>
      ...
      <div className="grid gap-4 lg:grid-cols-[13rem_minmax(0,1fr)]">
        ...
        <div>
          ...
          {!done && (
            <label className="mt-2 flex cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-brand-200 bg-white px-3 py-2 text-sm font-medium text-brand-700 hover:bg-brand-50">
              ...
              {uploading ? "Charh rahi hai…" : item.imageUrl ? "Tasveer badlein" : "Tasveer lagayein"}
        ...
        <form action={saveAction} className="space-y-3">
          ...
          <div>
            <Label htmlFor={`name-${item.id}`}>Product ka naam</Label>
            <Input
              id={`name-${item.id}`}
              name="name"
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div>
              <Label htmlFor={`unit-${item.id}`}>Ikai</Label>
            ...
            <div>
              <Label htmlFor={`qty-${item.id}`}>Kitne aaye</Label>
              <Input
                id={`qty-${item.id}`}
                name="opening_qty"
                type="number"
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <Label htmlFor={`trade-${item.id}`}>Trade rate</Label>
              <Input
                id={`trade-${item.id}`}
                name="purchase_price"
                type="number"
              ...
              <p className="mt-0.5 text-[11px] text-surface-500">Dabbe par nahi hota — bill se bharein ya khali chhoRein</p>
            </div>
            <div>
              <Label htmlFor={`sale-${item.id}`}>Sale rate</Label>
              <Input
                id={`sale-${item.id}`}
                name="selling_price"
                type="number"
            ...
            <div>
              <Label htmlFor={`mrp-${item.id}`}>Chhapi hui qeemat (MRP)</Label>
              ...
              {item.barcode && (
                <p className="mt-0.5 text-[11px]">
                  {item.barcodeSource === "scanner" ? (
                    <span className="text-emerald-700">scanner se — lakeerein parhi gayin</span>
                  ) : item.barcodeSource === "ai" ? (
                    <span className="text-amber-700">AI ne parha — jaanch lein</span>
                  ) : (
                    <span className="text-surface-500">haath se likha</span>
                  )}
                  {barcodeOk === false && <span className="text-red-700"> · check digit theek nahi</span>}
                </p>
              )}
            </div>
          </div>
