    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <h2 className="mb-3 font-display text-base font-semibold text-surface-900 dark:text-white">{t("mo_vehicle_status", lang)}</h2>
              ...
              return (
                <div key={v.id} className={`rounded-lg border p-3 text-sm ${isDue ? "border-amber-200 bg-amber-50" : "border-surface-200 bg-white dark:border-surface-800 dark:bg-surface-900"}`}>
                  <div className="flex items-center justify-between">
                    ...
                    {isDue && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">{t("mo_service_due", lang)}</span>}
                  </div>
          ...
          <h2 className="mb-3 font-display text-base font-semibold text-surface-900 dark:text-white">{t("mo_maintenance_history", lang)}</h2>
          <div className="overflow-hidden rounded-card border border-surface-200 bg-white shadow-card dark:border-surface-800 dark:bg-surface-900">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {logs.length === 0 && (
                  <tr><td colSpan={4} className="px-3 py-8 text-center text-surface-400">{t("mo_no_maintenance", lang)}</td></tr>
                )}
              </tbody>
            </table>
          </div>
    ...
    <div className="rounded-card border border-surface-200 bg-white p-4 shadow-card dark:border-surface-800 dark:bg-surface-900">
      ...
      <form action={formAction} className="space-y-2">
        ...
        <select name="maintenance_type" defaultValue="oil_change" className="w-full rounded-lg border border-surface-200 p-2 text-sm">
          ...
          <option value="repair">{t("mo_repair", lang)}</option>
          <option value="tyre">{t("c_tyre", lang)}</option>
          <option value="battery">{t("mo_battery", lang)}</option>
          <option value="other">{t("mo_other", lang)}</option>
        </select>
        <input type="number" step="0.1" name="km_at_service" required placeholder={t("mo_km_now", lang)} className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
        <textarea name="description" required rows={2} placeholder={t("mo_what_work", lang)} className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
        <input type="number" step="0.01" name="cost" placeholder={t("mo_cost_rs", lang)} className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900">{t("mo_withdraw_fund", lang)}</h3>
