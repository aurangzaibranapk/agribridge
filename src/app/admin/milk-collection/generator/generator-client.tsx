            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-200 bg-surface-50 text-left dark:border-surface-800 dark:bg-surface-800">
                  ...
                  <th className="px-3 py-2 text-right font-medium text-surface-500">Hours Run</th>
                  <th className="px-3 py-2 text-right font-medium text-surface-500">Diesel (L)</th>
              ...
              <tbody>
                ...
                {logs.length === 0 && (
                  <tr><td colSpan={7} className="px-3 py-8 text-center text-surface-400">Abhi koi generator log nahi hai.</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function GeneratorLogForm({ branches }: { branches: Branch[] }) {
  ...
  return (
    <div className="rounded-card border border-surface-200 bg-white p-4 shadow-card dark:border-surface-800 dark:bg-surface-900">
      ...
      {state.success && <p className="mb-2 rounded-lg bg-brand-50 px-3 py-2 text-xs text-brand-700">Log save ho gaya - cost khud calculate ho gaya.</p>}
      <form action={formAction} encType="multipart/form-data" className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <input type="number" step="0.1" name="opening_hours" required placeholder="Opening Hours (meter)" className="rounded-lg border border-surface-200 p-2 text-sm" />
          <input type="number" step="0.1" name="closing_hours" required placeholder="Closing Hours (meter)" className="rounded-lg border border-surface-200 p-2 text-sm" />
        </div>
        <input type="number" step="0.1" name="diesel_liters_purchased" placeholder="Diesel Liters" className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
        <p className="text-xs text-surface-400">Diesel Cost aaj ke rate se khud calculate hoga.</p>
        <input type="number" step="0.1" name="electricity_units" placeholder="Electricity Units (agar mixed hain)" className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
        <input type="number" step="0.1" name="milk_volume_chilled" placeholder="Milk Chilled (L)" className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
        <div>
          <label className="text-xs text-surface-500">Generator Meter Photo (optional)</label>
