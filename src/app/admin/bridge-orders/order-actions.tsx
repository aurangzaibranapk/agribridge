      ...
      {paymentMode && (
        <div className="text-xs">
          ...
          {remaining > 0 ? (
            ...
            <span className="ml-2 font-medium text-green-600">Payment Poori</span>
          )}
        </div>
      )}
      ...
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="flex items-center gap-2 font-display text-base font-semibold text-surface-900">
            <CreditCard className="h-4 w-4 text-brand-600" /> Payment Record Karein
          </h3>
        ...
        <form action={formAction} className="space-y-2">
          <input type="hidden" name="order_id" value={orderId} />
          <input type="number" step="0.01" name="amount" max={remaining} defaultValue={remaining} required placeholder="Amount (Rs)" className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
