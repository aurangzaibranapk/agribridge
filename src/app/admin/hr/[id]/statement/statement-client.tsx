...
export function StaffStatementClient(props: Props) {
  ...
  function statementText() {
    const lines = [
      ...
      "",
      ...props.entries.map(
        (e) => `${e.date} | ${e.description} | ${e.credit ? `Credit: Rs ${e.credit.toLocaleString()}` : `Debit: Rs ${e.debit.toLocaleString()}`} | Balance: Rs ${e.runningBalance.toLocaleString()}`
      ),
      "",
      `Total Credits: Rs ${props.totalCredit.toLocaleString()}`,
      `Total Debits: Rs ${props.totalDebit.toLocaleString()}`,
  ...
  function handleDownload() {
    ...
    a.href = url;
    a.download = `${props.staffName.replace(/\s+/g, "-")}-statement-${props.startDate}-to-${props.endDate}.txt`;
    a.click();
  ...
  function handleEmail() {
    window.location.href = `mailto:?subject=${encodeURIComponent(`${props.staffName} Khata Statement`)}&body=${encodeURIComponent(statementText())}`;
  }
    ...
    <div>
      ...
      <div className="rounded-card border border-surface-200 bg-white p-5 shadow-card dark:border-surface-800 dark:bg-surface-900">
        <div className="mb-4 border-b border-surface-100 pb-3 dark:border-surface-800">
          <h2 className="font-display text-lg font-semibold text-surface-900 dark:text-white">{props.staffName} {props.role ? `(${props.role})` : ""}</h2>
