...
export function BuyerStatementClient(props: Props) {
  const [showPayment, setShowPayment] = useState(false);

  function statementText() {
    const lines = [
      `${props.buyerName} - Statement`,
      `Period: ${props.startDate} to ${props.endDate}`,
      "",
      ...(props.bankName || props.bankAccountNumber
        ? [
            `Bank: ${props.bankName ?? "-"} | Title: ${props.bankAccountTitle ?? "-"} | Account #: ${props.bankAccountNumber ?? "-"} | IBAN: ${props.bankIban ?? "-"}`,
            "",
          ]
        : []),
      ...props.entries.map(
  ...
  function handleDownload() {
    ...
    a.href = url;
    a.download = `${props.buyerName.replace(/\s+/g, "-")}-statement-${props.startDate}-to-${props.endDate}.txt`;
    a.click();
  ...
  function handleEmail() {
    window.location.href = `mailto:?subject=${encodeURIComponent(`${props.buyerName} Statement`)}&body=${encodeURIComponent(statementText())}`;
  }
    ...
    <div>
      ...
      <div className="rounded-card border border-surface-200 bg-white p-5 shadow-card dark:border-surface-800 dark:bg-surface-900">
        <div className="mb-4 border-b border-surface-100 pb-3 dark:border-surface-800">
          <h2 className="font-display text-lg font-semibold text-surface-900 dark:text-white">{props.buyerName}</h2>
            ...
            {props.entries.map((e, i) => (
              <tr key={i} className="border-b border-surface-50 last:border-0 dark:border-surface-800">
                ...
                <td className="px-2 py-1.5">
                  {e.slipUrl ? (
                    <a href={e.slipUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-brand-600 hover:underline">
                      View
                    </a>
                  ) : (
                    "-"
      ...
      {showPayment && <PaymentModal buyerId={props.buyerId} onClose={() => setShowPayment(false)} />}
    </div>
  );
}

function PaymentModal({ buyerId, onClose }: { buyerId: string; onClose: () => void }) {
  const [state, formAction] = useFormState(recordBuyerPayment, initialState);
      ...
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        ...
        <form action={formAction} encType="multipart/form-data" className="space-y-2">
          ...
          <select name="direction" className="w-full rounded-lg border border-surface-200 p-2 text-sm">
            <option value="we_paid">Hum ne diya (We Paid)</option>
            <option value="they_paid">Unhone diya (They Paid)</option>
          </select>
