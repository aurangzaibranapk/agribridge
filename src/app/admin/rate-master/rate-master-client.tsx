...
export function RateMasterClient({ landPrepRates, laborRates }: { landPrepRates: Rate[]; laborRates: Rate[] }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <RateSection title={t("rm_land_prep", lang)} rates={landPrepRates} nameField="activity_name" rateField="rate_per_acre" saveAction={saveLandPrepRate} deleteAction={deleteLandPrepRate} placeholder={t("rm_eg_plough", lang)} />
      <RateSection title={t("rm_labour_rates", lang)} rates={laborRates} nameField="labor_type" rateField="rate" saveAction={saveLaborRate} deleteAction={deleteLaborRate} placeholder={t("rm_eg_spray", lang)} />
    </div>
  );
}
    ...
    <div className="rounded-card border border-surface-200 bg-white p-4 shadow-card dark:border-surface-800 dark:bg-surface-900">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1 rounded-lg bg-brand-600 px-2.5 py-1.5 text-xs font-medium text-white hover:bg-brand-700">
          <Plus className="h-3.5 w-3.5" />{t("c_add", lang)}</button>
      </div>
      ...
      {showForm && (
        <form action={formAction} className="mb-3 flex gap-2 rounded-lg bg-surface-50 p-3 dark:bg-surface-800">
          ...
          <input name={rateField} type="number" step="0.01" required placeholder={t("c_rate", lang)} className="w-24 rounded-lg border border-surface-200 p-2 text-sm dark:border-surface-700 dark:bg-surface-900" />
          <SubmitButton />
        </form>
      )}
      ...
      <div className="space-y-1.5">
        {rates.length === 0 ? (
          <p className="text-sm text-surface-400">{t("rm_none", lang)}</p>
        ) : (
          rates.map((r) => (
