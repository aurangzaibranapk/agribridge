  ...
  return (
    <div>
      <button onClick={() => setShowNewLoan(true)} className="mb-4 flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-2 text-sm font-medium text-white hover:bg-brand-700">
        <Plus className="h-4 w-4" /> Naya Loan Dein
      </button>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-surface-200 bg-surface-50 text-left dark:border-surface-800 dark:bg-surface-800">
              ...
              <th className="px-3 py-2 text-right font-medium text-surface-500">Loan Amount</th>
              <th className="px-3 py-2 text-right font-medium text-surface-500">Weekly Installment</th>
          ...
          <tbody>
            ...
            {loans.length === 0 && (
              <tr><td colSpan={6} className="px-3 py-8 text-center text-surface-400">Koi Loan nahi hai.</td></tr>
            )}
          </tbody>
        </table>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl dark:bg-surface-900">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900 dark:text-white">Naya Loan Dein</h3>
        ...
        <form action={formAction} className="space-y-3">
          ...
          <div>
            ...
            <Input type="number" step="0.01" name="principal_amount" required placeholder="jaise 10000" />
          </div>
          <div>
            <Label>Weekly Installment (Rs.) *</Label>
            <Input type="number" step="0.01" name="weekly_installment" required placeholder="jaise 500" />
          </div>
          <p className="text-[11px] text-surface-400">Loan amount turant Farmer ki Wallet mein chala jayega. Har hafte (automatic) is Wallet se Installment kat ke Loan kam hota rahega.</p>
          <div>
