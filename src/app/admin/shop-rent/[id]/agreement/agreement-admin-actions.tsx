    ...
    <div className="mb-4 flex flex-wrap items-center gap-2 print:hidden">
      ...
      {!hasCompanySignature && (
        <button onClick={() => setShowSign(true)} className="flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-2 text-xs font-medium text-white hover:bg-brand-700">
          <PenTool className="h-3.5 w-3.5" /> Company Sign Karein
        </button>
      )}
      <button onClick={copyLink} className="flex items-center gap-1.5 rounded-lg border border-surface-200 px-3 py-2 text-xs font-medium text-surface-600 hover:bg-surface-50">
        <Copy className="h-3.5 w-3.5" /> Landlord Link Copy Karein
      </button>
      <button onClick={() => setShowEmail(true)} className="flex items-center gap-1.5 rounded-lg border border-surface-200 px-3 py-2 text-xs font-medium text-surface-600 hover:bg-surface-50">
        <Mail className="h-3.5 w-3.5" /> Link Email Karein
      </button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-card bg-white p-5 shadow-xl">
        <h3 className="mb-3 font-display text-base font-semibold text-surface-900">Company Ki Taraf Se Sign Karein</h3>
        ...
        <form action={formAction}>
          ...
          <div className="mt-3 flex gap-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-lg border border-surface-200 py-2 text-sm text-surface-600 hover:bg-surface-50">Cancel</button>
            <SubmitButton disabled={!signatureData} label="Sign Save Karein" />
          </div>
        </form>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        <h3 className="mb-3 font-display text-base font-semibold text-surface-900">Landlord Ko Link Email Karein</h3>
        {state.error && <p className="mb-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{state.error}</p>}
        {state.success && <p className="mb-2 flex items-center gap-1 rounded-lg bg-brand-50 px-3 py-2 text-xs text-brand-700"><CheckCircle2 className="h-3.5 w-3.5" /> Email bhej di gayi.</p>}
        <form action={formAction} className="space-y-2">
          <input type="hidden" name="agreement_id" value={agreementId} />
          <input type="email" name="to_email" required placeholder="Landlord ka email" className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
          <div className="flex gap-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-lg border border-surface-200 py-2 text-sm text-surface-600 hover:bg-surface-50">Cancel</button>
            <SubmitButton disabled={false} label="Bhejein" />
          </div>
        </form>
      </div>
    </div>
