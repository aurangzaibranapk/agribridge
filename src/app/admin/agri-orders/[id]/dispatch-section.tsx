    ...
    <div className="rounded-card border border-surface-200 bg-white p-4 shadow-card dark:border-surface-800 dark:bg-surface-900">
      ...
      {!dispatch && orderStatus === "approved" && permissions.canCreateDispatch && (
        advanceBlocked ? (
          ...
          <button onClick={() => setShowCreate(true)} className="w-full rounded-lg bg-brand-600 py-2 text-sm font-medium text-white hover:bg-brand-700">{t("ao_create_dispatch", lang)}</button>
        )
      )}
      ...
      {dispatch && (
        <div className="space-y-1 text-sm">
          ...
          <div className="flex justify-between"><span className="text-surface-500">{t("c_vehicle", lang)}</span><span>{dispatch.vehicle_no ?? "-"}</span></div>
          <div className="flex justify-between"><span className="text-surface-500">{t("ao_driver", lang)}</span><span>{dispatch.driver_name ?? "-"} {dispatch.driver_mobile ? `(${dispatch.driver_mobile})` : ""}</span></div>
          <div className="flex justify-between"><span className="text-surface-500">{t("ao_dispatch_date", lang)}</span><span>{dispatch.dispatch_date}</span></div>
          {dispatch.delivery_location && <div className="flex justify-between"><span className="text-surface-500">{t("c_location", lang)}</span><span>{dispatch.delivery_location}</span></div>}

          {!delivery && (orderStatus === "dispatched" || orderStatus === "in_transit") && permissions.canConfirmDelivery && (
            <button onClick={() => setShowConfirm(true)} className="mt-3 w-full rounded-lg bg-brand-600 py-2 text-sm font-medium text-white hover:bg-brand-700">{t("ao_confirm_delivery", lang)}</button>
          )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900">{t("ao_create_dispatch", lang)}</h3>
        ...
        <form action={formAction} className="space-y-3">
          ...
          <div>
            <p className="mb-1 text-xs font-medium text-surface-600">{t("ao_items_adjust", lang)}</p>
            {orderItems.map((item) => (
              <div key={item.id} className="mb-2 rounded-lg border border-surface-200 p-2">
                ...
                <div className="mt-1 grid grid-cols-3 gap-1.5">
                  <div>
                    <label className="text-[10px] text-surface-400">{t("ao_dispatch_qty", lang)}</label>
          ...
          <SubmitButton label={t("ao_create_dispatch", lang)} />
        </form>
      </div>
    </div>
  );
...
function ConfirmDeliveryModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900">{t("ao_confirm_delivery", lang)}</h3>
          <button onClick={onClose} className="text-surface-400 hover:text-surface-700"><X className="h-5 w-5" /></button>
        </div>
        {state.error && <p className="mb-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{state.error}</p>}
        <form action={formAction} className="space-y-3">
          ...
          <div>
            <p className="mb-1 text-xs font-medium text-surface-600">{t("ao_check_each_item", lang)}</p>
              ...
              return (
                <div key={item.id} className="mb-2 rounded-lg border border-surface-200 p-2">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    <div>
                      <label className="text-[10px] text-surface-400">{t("c_received", lang)}</label>
                      <input
                        type="number"
                  ...
                  {hasDiff && (
                    <input
                      required
                      placeholder={t("ao_reason_required", lang)}
                      value={row?.reason ?? ""}
          ...
          <div>
            <label className="text-xs text-surface-500">{t("ao_receiver_note", lang)}</label>
            <input name="receiver_name" required defaultValue={currentUserIdentity.name} placeholder={t("ao_receiver_name", lang)} className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
          </div>
          ...
          <div>
            <label className="text-xs text-surface-500">{t("ao_delivery_challan", lang)}</label>
          ...
          <SubmitButton label={t("ao_confirm_delivery", lang)} />
        </form>
      </div>
    </div>
  );
