...
import { clearRoleTemplate, fillRoleTemplate, saveRoleTemplate, suggestRoleTemplate, type TemplateActionState, type TemplatePermissionDraft } from "@/actions/staff-access";
...
function ActionButton({ kind }: { kind: "save" | "ai" | "zero" | "all" }) {
...
export function TemplateManager({ templates, features }: { templates: Template[]; features: Feature[] }) {
  ...
  useEffect(() => { if (allState.suggestion) setRows(allState.suggestion); }, [allState.suggestion]);
  ...
  function setAllScopes(data_scope: DataScope) { setRows((before) => before.map((r) => ({ ...r, data_scope }))); }
  const message = allState.message ?? saveState.message ?? clearState.message ?? aiState.message;
  const error = allState.error ?? saveState.error ?? clearState.error ?? aiState.error;
    ...
    {open && <div className="border-t border-surface-100 p-4 dark:border-surface-800">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(14rem,20rem)_1fr]">
        <div className="space-y-3">
          ...
          <form action={allAction} onSubmit={(e) => { if (!window.confirm(`${selected?.label ?? role} template ko TAMAM active features aur TAMAM actions dene hain?`)) e.preventDefault(); }}><input type="hidden" name="role" value={role} /><ActionButton kind="all" /></form>
        ...
        <div className="rounded-lg border border-surface-200 p-3 dark:border-surface-700">
          ...
          {rows.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-800 dark:bg-amber-950/20"><span className="text-xs font-medium text-amber-800 dark:text-amber-300">Sab ki scope ek sath badlein:</span>{DATA_SCOPES.map((s) => <button key={s} type="button" onClick={() => setAllScopes(s)} className="rounded-md border border-amber-300 bg-white px-2 py-0.5 text-[11px] font-semibold text-amber-900 hover:bg-amber-100 dark:border-amber-700 dark:bg-surface-900 dark:text-amber-200">{SCOPE_LABEL[s]}</button>)}</div>}
