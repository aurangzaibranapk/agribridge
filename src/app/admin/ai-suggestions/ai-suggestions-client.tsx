  ...
  return (
    <div className="space-y-3">
      {suggestions.length === 0 && (
        <p className="rounded-card border border-dashed border-surface-200 py-10 text-center text-sm text-surface-400">{t("as_none", lang)}</p>
      )}
      {suggestions.map((s) => (
...
function SuggestionCard({ suggestion, canDecide }: { suggestion: Suggestion; canDecide: boolean }) {
  ...
  return (
    <div className="rounded-card border border-surface-200 bg-white p-4 shadow-card dark:border-surface-800 dark:bg-surface-900">
      ...
      <p className="text-sm text-surface-600 dark:text-surface-400">{t("as_suggested_qty", lang)}<strong>{suggestion.suggested_qty}</strong></p>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
          ...
          <>
            <form action={approveAction}>
              ...
              <button type="submit" className="flex items-center gap-1 rounded-lg bg-green-50 px-2 py-1 text-xs font-medium text-green-700 hover:bg-green-100">
                <CheckCircle2 className="h-3 w-3" />{t("as_approve", lang)}</button>
            </form>
            <button onClick={() => setShowReject(true)} className="flex items-center gap-1 rounded-lg bg-red-50 px-2 py-1 text-xs font-medium text-red-700 hover:bg-red-100">
              <XCircle className="h-3 w-3" />{t("c_reject", lang)}</button>
          </>
        )}
        {!canDecide && suggestion.status === "pending" && !suggestion.branch_comment && (
          <button onClick={() => setShowComment(true)} className="flex items-center gap-1 rounded-lg bg-surface-100 px-2 py-1 text-xs font-medium text-surface-600 hover:bg-surface-200">
            <MessageSquare className="h-3 w-3" />{t("as_add_comment", lang)}</button>
        )}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900">{t("as_reject_suggestion", lang)}</h3>
          <button onClick={onClose} className="text-surface-400 hover:text-surface-700"><X className="h-5 w-5" /></button>
        </div>
        <p className="mb-2 text-xs text-surface-400">{t("as_reason_remembered", lang)}</p>
...
function CommentModal({ suggestionId, onClose }: { suggestionId: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-card bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-semibold text-surface-900">{t("as_add_comment", lang)}</h3>
          <button onClick={onClose} className="text-surface-400 hover:text-surface-700"><X className="h-5 w-5" /></button>
        </div>
        <p className="mb-2 text-xs text-surface-400">{t("as_tell_if_needed", lang)}</p>
        {state.error && <p className="mb-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{state.error}</p>}
        <form action={formAction} className="space-y-2">
          <input type="hidden" name="suggestion_id" value={suggestionId} />
          <textarea name="branch_comment" required rows={3} placeholder={t("as_write_comment", lang)} className="w-full rounded-lg border border-surface-200 p-2 text-sm" />
          <SubmitButton />
        </form>
      </div>
    </div>
