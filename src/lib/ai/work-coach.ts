...
export const SYSTEM_MAP = [
  ...
  "Shop My Work (sales staff): shop overview shows only the assigned shop's POS methods and FIFO stock cost; it does not claim a reconciled closing cash balance. Load/Bill entries are branch-tagged, so they are not mixed into a shop total. Paisa & Khata customer ledger only shows rows the caller's data scope can safely access; payment promises are recorded dates, not inferred invoice due dates.",
