...
export const SYSTEM_MAP = [
  ...
  "Mustaqil asaasay (/admin/finance/assets): machine, gaari, computer, furniture -- khareed KHARCHA NAHI, qeemat saalon mein baanti jati hai (depreciation). Qism (/admin/finance/assets/categories) khata aur umar rakhti hai; har mahine /admin/finance/assets/depreciation par pehle hisaab (draft), phir 'Ledger mein daalein'; ek mahina ek dafa. Farokht aur dobara qeemat asaase ke apne safhe par. Machinery module ka kaam (booking, kiraya, operator) is se ALAG hai -- wahan kaam, yahan sirf qeemat aur ghisai",
  "Finance ka markaz /admin/finance/center: paanch group. Gosharay (/admin/finance/statements), Maali reports (/admin/finance/reports: cash flow, chalta sarmaya, khaton ka baqi, aaya-gaya, kis se lena kis ko dena, shaakh ka nafa nuqsan), Khaton ki fehrist (/admin/finance/accounts), Hisaab ke arse (/admin/finance/periods: band mahine mein entry nahi jati), Budget (/admin/finance/budget), Cheque (/admin/finance/cheques: cheque cash tab banta hai jab bank se guzre), Har mahine wali entry (/admin/finance/recurring: khud nahi banti, ek dabao par), Adaigi ki shartein (/admin/finance/terms), Cheez ki lagat (/admin/finance/costing), Mustaqil asaasay (/admin/finance/assets)",
  ...
  "Data Health (/admin/data-health, 396): roz khud check karta hai -- machinery ka dobara payout, booking ka bill se mel na khana, stock ledger ka farq, manfi cash/bank. fn_data_health_scan() naye masle data_health_findings mein daalta hai (dedupe_key se dohra nahi banta). Har department apna hissa dekhta hai; koi bhi masla khud theek nahi hota -- sirf 'Claude ko bhejo', 'theek ho gaya', ya 'chhoड़ do' (wajah ke sath), anomaly_findings (114) se alag hai: wo pattern/tarteeb dekhta hai (kam az kam 5 aankre), ye single-event bookkeeping ghaltiyan",
].join("\n");
