...
export interface ShopDailySalesPoint {
  date: string;
  label: string;
  total: number;
}
...
/**
 * Pichle `days` dinon ki daily sale — Shop Overview ka trend chart.
 * `pos_sales.total_amount` (discount ke baad) ko date ke hisaab se jama
 * karta hai. Chart mein sirf wo din jismein sale zero bhi ho — takhliya
 * nahi chhoda jata.
 */
export async function shopDailySalesTrend(shopId: string, days: number): Promise<ShopDailySalesPoint[]> {
  const service = createServiceClient();
  const today = new Date();
  const result: ShopDailySalesPoint[] = [];
  ...
  for (const r of (rows ?? []) as { created_at: string; total_amount: number | null }[]) {
    const date = String(r.created_at).slice(0, 10);
    byDate.set(date, round2((byDate.get(date) ?? 0) + Number(r.total_amount ?? 0)));
  }
  ...
  for (let i = 0; i < days; i++) {
    ...
    result.push({ date: dateStr, label, total: byDate.get(dateStr) ?? 0 });
  }
