...
class _CartScreenState extends ConsumerState<CartScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Review Order')), body: cart.isEmpty ? const Center(child: Text('Cart abhi khali hai.')) : ListView(padding: const EdgeInsets.all(16), children: [
      ...
      DropdownButtonFormField<String>(value: payment, items: ['Cash on Delivery', 'Shop Pickup Payment', 'Customer Khata', 'Bank Transfer', 'Easypaisa', 'JazzCash', 'Advance Payment'].map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(), onChanged: (v) => setState(() => payment = v!)),
