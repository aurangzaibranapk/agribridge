-- Secure mobile ordering foundation (Testing first).
-- Product prices are always re-read server-side; the app cannot forge totals.
...
create policy mobile_read_own_agri_order_items on public.agri_order_items
  for select to authenticated using (
    exists (
      select 1 from public.agri_orders o
      where o.id = agri_order_items.order_id and o.requested_by = auth.uid()
    )
  );
...
create policy mobile_read_own_agri_order_timeline on public.agri_order_timeline
  for select to authenticated using (
    exists (
      select 1 from public.agri_orders o
      where o.id = agri_order_timeline.order_id and o.requested_by = auth.uid()
    )
  );
