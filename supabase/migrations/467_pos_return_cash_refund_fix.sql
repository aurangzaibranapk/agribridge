-- =====================================================================
-- AgriBridge — Migration 467: Wapsi ka cash hisaab durust karna
-- =====================================================================
-- Masla (25 September): Shift Close ka Expected Cash, "original" tareeqa
-- se ki gayi wapsi ghatata nahi tha.
--
-- Wajah 1 (SQL bug): fn_pos_return_lines ka `v_cash` SAARE non-khata
-- tareeqon ka jama ho jata tha — cash bhi, bank bhi, digital bhi.
-- `cash_refund` mein sab daal deta tha, jabke sirf actual cash ka hissa
-- rehna chahiye tha.
--
-- Wajah 2 (TypeScript): aggregateShiftCash sirf `refund_method = 'cash'`
-- ya NULL wali wapsi ghatata tha; "original" method ko chhoR deta tha.
-- Wahan TypeScript mein theek kiya gaya hai (shift-cash.ts).
--
-- Yahan sirf SQL theek ho raha hai: for "original" refund, sirf cash
-- account wali finance_transactions hi v_cash mein jati hain.
-- =====================================================================
...
begin
  ...
  if coalesce(p_refund_method, 'original') = 'cash' then
    ...
    -- "Original" method: har asal tareeqe ka hissa wapas -- finance transaction
    -- har tareeqe ke liye alag alag. Sirf CASH wali transactions v_cash mein
    -- jati hain -- bank/digital wapis un ke apne khaaton mein jate hain.
    -- (467: pehle v_cash mein saare non-khata methods aa jate the -- ghalat)
    for v_pay in
    ...
    loop
      if v_pay.finance_account_id is not null then
        ...
        -- Sirf CASH wali miqdar golak se nikaali — baki tareeqe apne khaaton se
...
comment on function public.fn_pos_return_lines(uuid, jsonb, text, text, text, text, text, uuid) is
  'POS ek-ek cheez ki wapsi (295/300, 380: shift_id capture, 467: "original" method mein cash_refund sirf cash portion, digital alag).';
